...
public class AbstractFactorySimulator {
  ...
  private static Source readTypeOfSource() {
    ...
    BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
    System.out
        .print("1 if you want work with csv file, 2 if you want work with db");
    try {
  ...
  private static int readCurrrentAction() {
    ...
    while (true) {
      BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
      System.out
          .print("Please check operation: 1- read all person ; 2- find person by name; 3- write person; 4-exit");
      try {
        i = Integer.parseInt(br.readLine());
      } catch (NumberFormatException nfe) {
        System.err.println("Invalid Format!");
